...
from .exceptions import TrayAuthenticationError, TrayConnectionError, TrayError
from .tray_rate_limit import acquire_tray_quota
from .tray_token_store import load_token_state, save_token_state, token_refresh_lock

logger = logging.getLogger(__name__)

_REFRESH_AHEAD_SECONDS = 300
_DEFAULT_ACCESS_TTL_SECONDS = 7200
...
class TokenState:
    ...
    def access_is_valid(self, *, skew_seconds: int = 30) -> bool:
        if not self.access_token:
            return False
        # Null expiry is not trustable (stale access after cold start / partial cache).
    ...
    def should_refresh_proactively(self) -> bool:
        if not self.access_token or not self.refresh_token:
            return True
        # Unknown expiry must refresh — treating None as forever caused 503 bursts.
        if self.access_expires_at is None:
            return True
        return self.access_expires_at <= datetime.now(timezone.utc) + timedelta(
    ...
    def refresh_is_usable(self, *, skew_seconds: int = 30) -> bool:
        if not self.refresh_token:
            return False
        if self.refresh_expires_at is None:
            return True
        return self.refresh_expires_at > datetime.now(timezone.utc) + timedelta(
            seconds=skew_seconds
        )
...
class TrayAuth:
    ...
    async def refresh(self) -> TokenState:
        ...
        acquire_tray_quota(self.settings)
        try:
            if self.http_client:
    ...
    async def get_valid_token(self) -> TokenState:
        async with self._lock:
            if (
                self.state
                and self.state.access_is_valid()
                and not self.state.should_refresh_proactively()
            ):
                return self.state

            self._hydrate_from_cache(load_token_state(self.settings))
            if (
                self.state
                and self.state.access_is_valid()
                and not self.state.should_refresh_proactively()
            ...
            with token_refresh_lock(self.settings):
                self._hydrate_from_cache(load_token_state(self.settings), overwrite=True)
                if (
                    self.state
                    and self.state.access_is_valid()
                    and not self.state.should_refresh_proactively()
                    ...
                    return self.state
                return await self._refresh_or_authenticate()

    async def _refresh_or_authenticate(self) -> TokenState:
        if self.state and self.state.refresh_is_usable():
            try:
        ...
        if cached and cached.get("refresh_token") and not self.state:
            self.state = TokenState(
                ...
                access_expires_at=None,
                refresh_expires_at=cached.get("refresh_expires_at"),
                api_host=None,
                store_id=str(self.settings.tray_store_code),
            )
            if self.state.refresh_is_usable():
                try:
                    return await self.refresh()
                except TrayError as exc:
    ...
    def _hydrate_from_cache(
        self, cached: dict[str, Any] | None, *, overwrite: bool = False
    ) -> None:
        if not cached:
            return
        if self.state and not overwrite:
            if self.state.access_token or self.state.refresh_token:
                if self.state.refresh_expires_at is None and cached.get("refresh_expires_at"):
                    self.state.refresh_expires_at = cached.get("refresh_expires_at")
                return
        self.state = TokenState(
    ...
    def _parse_token_response(self, body: dict[str, Any]) -> TokenState:
        ...
        access_expires_at = _parse_tray_datetime(
            body.get("date_expiration_access_token")
        ) or _expiry(body, "expires_in", "access_token_expires_in")
        if access_expires_at is None:
            # Tray occasionally omits expires_in; persist a finite TTL so proactive
            # refresh and cache stay honest (default 2h matches common Tray windows).
            access_expires_at = datetime.now(timezone.utc) + timedelta(
                seconds=_DEFAULT_ACCESS_TTL_SECONDS
            )
        refresh_expires_at = _parse_tray_datetime(
            body.get("date_expiration_refresh_token")
        ) or _expiry(body, "refresh_expires_in", "refresh_token_expires_in")
        return TokenState(
...
def _parse_tray_datetime(value: Any) -> datetime | None:
    if value is None:
        return None
    ...
    for fmt in ("%Y-%m-%d %H:%M:%S", "%Y-%m-%dT%H:%M:%S"):
        try:
            parsed = datetime.strptime(text, fmt).replace(tzinfo=_SAO_PAULO)
            return parsed.astimezone(timezone.utc)
        except ValueError:
            continue
    try:
